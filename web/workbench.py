...
for idx, (schema_name, schema) in enumerate(dask_sql_context.schema.items()):
    with st.sidebar.expander(schema_name):
        ...
        choosed_fn = st.selectbox(
            label="Functions", options=schema.functions, key=f"{idx}_Functions"
        )
        st.subheader("Experiments")
        choosed_exp = st.selectbox(
            label="Experiments", options=schema.experiments, key=f"{idx}_Experiments"
        )
        st.subheader("Models")
        choosed_model = st.selectbox(
            label="Models", options=schema.models, key=f"{idx}_Models"
        )


create_udf_function = st.sidebar.radio(
    "Create Python UDF", options=("No", "Yes"), help=udf_help_text
)
